...
public class TSHLambdaBlock extends TSHBlock {
    public boolean isSynchronized = false;

    public TSHLambdaBlock(String id) {
        super(id);
    }

    public Object evalBlock(CallStack callstack, Interpreter interpreter, boolean overrideNamespace, NodeFilter nodeFilter) throws EvalError {
            ...
            if (numChildren == 1) {
                ...
                for (int i = startChild; i < numChildren; i++) {
                    ...
                    if (nodeFilter != null && !nodeFilter.isVisible(node))
                        continue;
                    Object temp = node.eval(callstack, interpreter);
                    list.add(temp);
                }
                ret = list;
            }
